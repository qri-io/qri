...
func NewQriNode(r repo.Repo, options ...func(o *config.P2P)) (node *QriNode, err error) {
	...
	if len(cfg.PeerID) == 0 || len(cfg.PrivKey) == 0 {
		// Neither tests nor production should ever be running this. Keys should
		// either come fakes for tests, or the config generated by setup for prod.
		// Needed at the moment for tests to pass.
		// TODO: Remove this, require NewQriNode to be passed a config with PrivKey and PeerID set.
		log.Errorf("WARNING: Generating Key and PeerID, this is really SLOW")
