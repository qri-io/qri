...
func TestRegisterProfile(t *testing.T) {
	...
	cases := []struct {
		...
		{&Profile{ProfileID: p.ProfileID, Username: p.Username, Signature: p.Signature, PublicKey: "bad_data"}, "decoding base64-encoded public key: illegal base64 data at input byte 3"},
		{&Profile{ProfileID: p.ProfileID, Username: p.Username, Signature: p.Signature, PublicKey: base64.StdEncoding.EncodeToString([]byte("bad_data"))}, `public key "YmFkX2RhdGE=" is invalid: unexpected EOF`},
