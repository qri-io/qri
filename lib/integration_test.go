...
func TestReferencePulling(t *testing.T) {
	...
	logRes, err := nasim.Log().Log(tr.Ctx, &LogParams{Ref: ref.Alias(), ListParams: ListParams{Limit: 1}})
	if err != nil {
		t.Fatal(err)
	}
...
`
	...
	logRes, err = adnan.Log().Log(tr.Ctx, &LogParams{Ref: ref.Alias(), ListParams: ListParams{Limit: 1}})
	if err != nil {
		t.Fatal(err)
	}
