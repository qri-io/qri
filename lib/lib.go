...
func NewInstance(ctx context.Context, repoPath string, opts ...Option) (qri *Instance, err error) {
	...
	if o.logAll {
		allPackages := []string{"qriapi", "qrip2p", "base", "changes", "cmd", "config", "dsref", "dsfs", "friendly", "fsi", "lib", "logbook", "profile", "repo"}
	...
	if cfg.RPC.Enabled {
		...
		if err == nil {
			...
			maAddr, err := ma.NewMultiaddr(cfg.API.Address)
			if err != nil {
				return nil, err
			}
			...
			inst.http = NewHTTPClient(httpAddr.String())
...
// HTTP accesses the instance HTTP client if one exists
func (inst *Instance) HTTP() *HTTPClient {
	if inst == nil {
		return nil
	}
	return inst.http
